...
// SetUsernameLookup installs the callback used to resolve display names for
// registered devices.
func (r *DeviceRegistry) SetUsernameLookup(fn func(userID string) string) {
	r.lookupUsername = fn
}
...
func (r *DeviceRegistry) UpdatePlayback(client *WSClient, snap PlaybackSnapshot) {
	...
	if !dev.isActivePlayer && !snap.Paused && snap.TrackID != "" {
		...
		for _, other := range r.devices {
			if other.scopeKey == client.ScopeKey() && other.isActivePlayer {
				hasActive = true
				break
			}
		}
	...
	if active {
		r.broadcastToScope(scope, Event{
			...
			Payload: map[string]any{"deviceId": client.DeviceID(), "state": snap},
		})
	}
	if sessionID != "" && isHost {
...
func (r *DeviceRegistry) TakeOver(client *WSClient) string {
	return r.grantPlayback(client, client.DeviceID())
}
...
func (r *DeviceRegistry) grantPlayback(client *WSClient, targetID string) string {
	r.mu.Lock()
	scope := client.ScopeKey()
	source := r.devices[deviceKey(scope, client.DeviceID())]
