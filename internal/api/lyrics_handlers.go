...
func (s *Server) registerLyricsRoutes() {
	s.mux.HandleFunc("GET /api/music/settings/lyrics", s.handleGetLyricsSettings)
	s.mux.HandleFunc("PUT /api/music/settings/lyrics", s.handlePutLyricsSettings)
	s.mux.HandleFunc("GET /api/music/lyrics/{trackId}", s.handleGetTrackLyrics)
	s.mux.HandleFunc("POST /api/music/lyrics/{trackId}/fetch", s.handleFetchTrackLyrics)
	s.mux.HandleFunc("POST /api/music/lyrics/{trackId}/whisper", s.handleWhisperTrackLyrics)
	s.mux.HandleFunc("DELETE /api/music/lyrics/cache", s.handleClearLyricsCache)
}

func (s *Server) handleGetLyricsSettings(w http.ResponseWriter, r *http.Request) {
	...
	httputil.WriteJSON(w, http.StatusOK, map[string]any{
		...
		"defaultStorageDir":  defaultLyricsStorageDir(s.cfg.DataDir),
		"autoFetch":          settings.AutoFetch,
		"providers":          settings.Providers,
		"builtinProviders":   lyrics.BuiltInProviders(),
		"whisperUrl":         settings.WhisperURL,
		"whisperEnabled":     extensions.IsEnabled(s.cfg.DataDir, lyricsWhisperExtensionID),
		"trackCount":         count,
		"usedBytes":          bytes,
	})
}

func (s *Server) handlePutLyricsSettings(w http.ResponseWriter, r *http.Request) {
	userID := ResolveProgressUserID(r.Context())
	...
	settings, err := mergeLyricsSettings(body, s.cfg.DataDir)
	if err != nil {
	...
	if err := s.preferences.Set(userID, store.PrefKeyLyricsSettings, string(encoded)); err != nil {
...
func (s *Server) handleGetTrackLyrics(w http.ResponseWriter, r *http.Request) {
	...
	doc, err := s.resolveTrackLyrics(r, in, false)
	if err != nil {
...
func (s *Server) handleFetchTrackLyrics(w http.ResponseWriter, r *http.Request) {
	trackID := strings.TrimSpace(r.PathValue("trackId"))
	if trackID == "" {
		writeLyricsError(w, http.StatusBadRequest, "track id required")
		return
	...
	in := lyricsFetchInputFromQuery(r, trackID)
	doc, err := s.resolveTrackLyrics(r, in, true)
	if err != nil {
		writeLyricsError(w, http.StatusNotFound, err.Error())
		return
	}
...
func (s *Server) handleClearLyricsCache(w http.ResponseWriter, r *http.Request) {
	userID := ResolveProgressUserID(r.Context())
	settings, err := s.loadLyricsSettings(userID)
	if err != nil {
		writeLyricsError(w, http.StatusInternalServerError, "failed to load lyrics settings")
...
func (s *Server) resolveTrackLyrics(r *http.Request, in lyrics.FetchInput, forceFetch bool) (*lyrics.Document, error) {
	if in.TrackID == "" {
	...
	subsonicFn := s.subsonicLyricsFetcher(r)
	providers := s.enabledLyricsProviders(settings, subsonicFn)
...
func (s *Server) subsonicLyricsFetcher(r *http.Request) lyrics.SubsonicFetcher {
