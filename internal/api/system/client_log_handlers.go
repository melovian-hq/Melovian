...
func (h *Handler) registerClientLogRoutes(mux *http.ServeMux) {
	mux.HandleFunc("POST /api/client-log", h.handleClientLog)
}
...
func (h *Handler) handleClientLog(w http.ResponseWriter, r *http.Request) {
	...
	if h.clientLogLimiter != nil {
		key := "clientlog"
		if addr, ok := apishared.ClientIP(r, h.cfg.TrustProxy); ok {
			key += "|" + addr.String()
		}
		if h.clientLogLimiter.Blocked(key) {
			apishared.WriteRateLimited(w, h.clientLogLimiter.RetryAfterSeconds(key))
			return
		}
		h.clientLogLimiter.Record(key)
	}
	...
	h.appendClientLog(entry)
	h.logClientEntry(r.Context(), entry, requestID)
...
func (h *Handler) appendClientLog(entry melog.ClientLogEntry) {
	path := filepath.Join(h.cfg.DataDir, "logs", "client.log")
...
func (h *Handler) logClientEntry(ctx context.Context, entry melog.ClientLogEntry, requestID string) {
	...
	melog.Default().Log(ctx, level, "client log", attrs...)
	if entry.Stack != "" && level >= slog.LevelWarn {
		...
		melog.Default().Log(ctx, level, "client stack", stackAttrs...)
	}
}
