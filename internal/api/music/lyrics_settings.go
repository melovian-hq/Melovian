...
func (h *LyricsHandler) loadLyricsSettings(userID string) (LyricsSettings, error) {
	raw, err := h.preferences.Get(userID, store.PrefKeyLyricsSettings)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return defaultLyricsSettings(h.cfg.DataDir), nil
		}
		return LyricsSettings{}, err
	}
	return MergeLyricsSettings(json.RawMessage(raw), h.cfg.DataDir)
}

func (h *LyricsHandler) LyricsRoot(settings LyricsSettings) string {
	root := strings.TrimSpace(settings.StorageDir)
	if root == "" {
		return DefaultLyricsStorageDir(h.cfg.DataDir)
	}
	if !filepath.IsAbs(root) {
		root = filepath.Join(h.cfg.DataDir, root)
		root = filepath.Clean(root)
		if err := osutil.PathEscapesRoot(h.cfg.DataDir, root); err != nil {
			return DefaultLyricsStorageDir(h.cfg.DataDir)
		}
		return root
	}
...
func (h *LyricsHandler) enabledLyricsProviders(settings LyricsSettings, subsonicFn lyrics.SubsonicFetcher) []lyrics.Provider {
