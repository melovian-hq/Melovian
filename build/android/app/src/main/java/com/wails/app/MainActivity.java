...
public class MainActivity extends AppCompatActivity {
            ...
            public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {
                ...
                if (request.getUrl().getHost() != null &&
                    ...
                    if (path != null && path.startsWith("/wails/")) {
                        ...
                        return bridge.serveWebResponse(fullPath, request.getMethod(), request.getRequestHeaders());
                    }
                    ...
                    if (path != null && path.startsWith("/api/")) {
                        ...
                        byte[] body = WailsJSBridge.takeRequestBody(headerValue(headers, "X-Wails-Body-Id"));
                        return bridge.serveWebResponse(fullPath, request.getMethod(), headers, body);
                    }
    ...
    private void injectEnvironmentFallback() {
        ...
        webView.evaluateJavascript(fetchBodyBridgeJs(), null);
    }

    private static String fetchBodyBridgeJs() {
        return "(function(){"
                + "if(window.__melFetchPatched)return;"
                + "window.__melFetchPatched=true;"
                + "var orig=window.fetch.bind(window);"
                + "window.fetch=function(input,init){"
                + "var args=arguments;"
                + "var req=(input instanceof Request && init==null)?input:new Request(input,init);"
                + "var url=new URL(req.url,location.href);"
                + "if(url.host!==location.host)return orig.apply(this,args);"
                + "var method=(req.method||'GET').toUpperCase();"
                + "if(method==='GET'||method==='HEAD')return orig.apply(this,args);"
                + "if(!window.wails||typeof window.wails.stashRequestBody!=='function')return orig.apply(this,args);"
                + "return req.text().then(function(body){"
                + "if(!body)return orig.apply(window,args);"
                + "var id='b'+Date.now().toString(36)+Math.random().toString(36).slice(2);"
                + "window.wails.stashRequestBody(id,body);"
                + "var headers=new Headers(req.headers);"
                + "headers.set('X-Wails-Body-Id',id);"
                + "return orig(url.href,{method:req.method,headers:headers,credentials:req.credentials,cache:req.cache,redirect:req.redirect,mode:req.mode,signal:req.signal});"
                + "});"
                + "};"
                + "})();";
    }
    ...
    private void showStartupError(String message) {
        ...
        String html = "<!doctype html><html><head>"
                ...
                + "<style>"
                + "body{margin:0;padding:24px;background:#090909;color:#f3f4f6;"
                + "font-family:system-ui,sans-serif;line-height:1.5}"
                + "h1{font-size:1.1rem;margin:0 0 12px}"
                + "p{margin:0;color:#9ca3af}"
                + "</style></head><body>"
                + "<h1>Melovian could not start</h1>"
                + "<p>" + safe + "</p>"
                + "</body></html>";
