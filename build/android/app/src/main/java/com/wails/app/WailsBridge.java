...
public class WailsBridge {
    ...
    private static volatile WailsBridge activeBridge;
    ...
    private static native void nativeInit(WailsBridge bridge, String dataDir, String cacheDir);
    ...
    private static native byte[] nativeServeAsset(String path, String method, String headers, byte[] body);
    ...
    public static void dispatchMediaAction(String action) {
        WailsBridge bridge = activeBridge;
        if (bridge == null) {
            return;
        }
        bridge.emitMediaAction(action);
    }
    ...
    private void emitMediaAction(String action) {
        ...
        // native-media-action is the shared name. android-media-action stays for older handlers.
        String js =
                "(function(){var d={detail:{action:'"
                        + safe
                        + "'}};window.dispatchEvent(new CustomEvent('native-media-action',d));"
                        + "window.dispatchEvent(new CustomEvent('android-media-action',d));})();";
    ...
    public void initialize() {
        try {
            ...
            String dataPath = files != null ? files.getAbsolutePath() : "";
            String cachePath = cache != null ? cache.getAbsolutePath() : dataPath;
            applyNativeEnvironment(files, dataPath, cachePath);
            nativeInit(this, dataPath, cachePath);
            initialized = true;
    ...
    /**
     * Point libc HOME/XDG/TMP at the app files dir. Go still needs the same
     * paths passed into nativeInit, because loadLibrary already started the
     * runtime and copied environ before this method runs.
     */
    private void applyNativeEnvironment(File files, String dataPath, String cachePath) {
        if (files == null) {
            Log.e(TAG, "getFilesDir() returned null");
            return;
        }
        if (!files.exists() && !files.mkdirs()) {
            Log.e(TAG, "could not create files dir: " + dataPath);
        }
        try {
            Os.setenv("HOME", dataPath, true);
            Os.setenv("TMPDIR", cachePath, true);
            Os.setenv("TMP", cachePath, true);
            Os.setenv("XDG_DATA_HOME", dataPath, true);
            Os.setenv("XDG_CONFIG_HOME", dataPath, true);
            Os.setenv("XDG_CACHE_HOME", cachePath, true);
            Os.setenv("XDG_STATE_HOME", dataPath, true);
            Os.setenv("MELOVIAN_DATA", dataPath, true);
            Log.i(TAG, "Native data dir: " + dataPath);
        } catch (ErrnoException e) {
            Log.e(TAG, "Failed to set native environment", e);
        }
    }
    ...
    public boolean isBackendReady() {
        ...
        try {
            PackedAsset probe = unpackAsset(nativeServeAsset("/index.html", "GET", "{}", null));
            return probe != null && probe.body != null && probe.body.length > 0;
        } catch (Exception e) {
    ...
    /**
     * Serve an asset from the Go asset server. Returns the response body after
     * unpacking the JJAR envelope (status + headers + body).
     */
    public byte[] serveAsset(String path, String method, String headers) {
        PackedAsset packed = servePackedAsset(path, method, headers, null);
        return packed == null ? null : packed.body;
    }

    public WebResourceResponse serveWebResponse(String path, String method, Map<String, String> requestHeaders) {
        return serveWebResponse(path, method, requestHeaders, null);
    }

    public WebResourceResponse serveWebResponse(String path, String method, Map<String, String> requestHeaders, byte[] requestBody) {
        String headersJson = "{}";
        if (requestHeaders != null && !requestHeaders.isEmpty()) {
            headersJson = new JSONObject(requestHeaders).toString();
        }
        PackedAsset packed = servePackedAsset(path, method, headersJson, requestBody);
        if (packed == null) {
            return new WebResourceResponse("text/plain", "UTF-8", 503, "Unavailable",
                    new HashMap<>(), new java.io.ByteArrayInputStream(new byte[0]));
        }
        String mime = packed.headers.get("Content-Type");
        if (mime == null || mime.isEmpty()) {
            mime = getAssetMimeType(path);
        }
        String encoding = null;
        int charset = mime.toLowerCase(Locale.US).indexOf("charset=");
        if (charset >= 0) {
            encoding = mime.substring(charset + 8).trim();
            int semi = encoding.indexOf(';');
            if (semi >= 0) encoding = encoding.substring(0, semi).trim();
            mime = mime.substring(0, mime.indexOf(';')).trim();
        }
        int status = packed.status > 0 ? packed.status : 200;
        String reason = status == 206 ? "Partial Content" : (status == 200 ? "OK" : "Error");
        Map<String, String> headers = new HashMap<>(packed.headers);
        headers.putIfAbsent("Access-Control-Allow-Origin", "*");
        byte[] body = packed.body != null ? packed.body : new byte[0];
    ...
    private PackedAsset servePackedAsset(String path, String method, String headers, byte[] body) {
        if (!initialized) {
        ...
        try {
            return unpackAsset(nativeServeAsset(path, method, headers, body));
        } catch (Exception e) {
    ...
    private static final class PackedAsset {
        int status;
        Map<String, String> headers = new HashMap<>();
        byte[] body = new byte[0];
    }

    private static PackedAsset unpackAsset(byte[] packed) {
        PackedAsset out = new PackedAsset();
        if (packed == null || packed.length == 0) {
            out.status = 404;
            return out;
        }
        if (packed.length >= 12
                && packed[0] == 'J' && packed[1] == 'J'
                && packed[2] == 'A' && packed[3] == 'R') {
            ByteBuffer buf = ByteBuffer.wrap(packed).order(ByteOrder.BIG_ENDIAN);
            buf.getInt();
            out.status = buf.getInt();
            int headerLen = buf.getInt();
            if (headerLen < 0 || 12 + headerLen > packed.length) {
                out.status = 500;
                return out;
            }
            if (headerLen > 0) {
                String json = new String(packed, 12, headerLen, StandardCharsets.UTF_8);
                try {
                    JSONObject obj = new JSONObject(json);
                    java.util.Iterator<String> keys = obj.keys();
                    while (keys.hasNext()) {
                        String key = keys.next();
                        out.headers.put(key, obj.optString(key, ""));
                    }
                } catch (Exception ignored) {
                }
            }
            int bodyOff = 12 + headerLen;
            out.body = java.util.Arrays.copyOfRange(packed, bodyOff, packed.length);
            return out;
        }
        out.status = 200;
        out.body = packed;
    ...
    public void startForegroundService(final String json) {
        mainHandler.post(() -> {
            try {
                ...
                if (o.has("title")) i.putExtra(WailsForegroundService.EXTRA_TRACK, o.optString("title"));
                if (o.has("artist")) i.putExtra(WailsForegroundService.EXTRA_ARTIST, o.optString("artist"));
                if (o.has("album")) i.putExtra(WailsForegroundService.EXTRA_ALBUM, o.optString("album"));
                if (o.has("artwork")) i.putExtra(WailsForegroundService.EXTRA_ARTWORK, o.optString("artwork"));
                if (o.has("playing")) i.putExtra(WailsForegroundService.EXTRA_PLAYING, o.optBoolean("playing"));
                if (o.has("duration")) i.putExtra(WailsForegroundService.EXTRA_DURATION_MS, (long) (o.optDouble("duration") * 1000));
                if (o.has("position")) i.putExtra(WailsForegroundService.EXTRA_POSITION_MS, (long) (o.optDouble("position") * 1000));
    ...
    /**
     * Store the WebView queue for Android Auto MediaBrowserService browse trees.
     * JSON: {"index":0,"tracks":[{"id","title","artist","album","artwork"},...]}
     */
    public void setMediaQueue(final String json) {
        mainHandler.post(() -> {
            try {
                JSONObject o = new JSONObject(json != null ? json : "{}");
                int index = o.optInt("index", -1);
                JSONArray tracks = o.optJSONArray("tracks");
                java.util.ArrayList<MelovianMediaSession.QueueItem> items =
                        new java.util.ArrayList<>();
                if (tracks != null) {
                    int n = Math.min(tracks.length(), 80);
                    for (int i = 0; i < n; i++) {
                        JSONObject t = tracks.optJSONObject(i);
                        if (t == null) continue;
                        items.add(new MelovianMediaSession.QueueItem(
                                t.optString("id", ""),
                                t.optString("title", ""),
                                t.optString("artist", ""),
                                t.optString("album", ""),
                ...
                MelovianMediaSession.setQueue(items, index);
                MelovianMediaBrowserService.notifyQueueChanged();
                MelovianMediaSession.getOrCreate(activity);
            } catch (Exception e) {
