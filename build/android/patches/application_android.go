...
// Overlay of Wails v3.0.0-beta.8 application_android.go with androidMainOnce
// (Activity recreation must not run main() again) and JJAR-packed asset
// responses so Range requests do not force the WebView to load a whole track.
...
static jbyte* jbyteArrayElems(JNIEnv *env, jbyteArray arr, jint *len) {
    if (len != NULL) {
        *len = 0;
    }
    if (arr == NULL) {
...
static void releaseJByteArray(JNIEnv *env, jbyteArray arr, jbyte *elems) {
    if (arr != NULL && elems != NULL) {
...
var (
	...
	// Activity recreation re-runs nativeInit. Start main() only once so
	// flag registration and listeners are not duplicated.
...
// applyAndroidProcessEnv writes HOME/XDG/MELOVIAN_DATA into Go's environment.
// Java Os.setenv after System.loadLibrary does not update the copy Go took
// when the shared library started, so these must be set here before main().
func applyAndroidProcessEnv(dataDir, cacheDir string) {
	if dataDir == "" {
		androidLogf("error", "nativeInit: empty data dir")
		return
	}
	if cacheDir == "" {
		cacheDir = dataDir
	...
	_ = os.Setenv("TMP", cacheDir)
	_ = os.Setenv("XDG_DATA_HOME", dataDir)
	_ = os.Setenv("XDG_CONFIG_HOME", dataDir)
	_ = os.Setenv("XDG_CACHE_HOME", cacheDir)
	_ = os.Setenv("XDG_STATE_HOME", dataDir)
	_ = os.Setenv("MELOVIAN_DATA", dataDir)
	androidLogf("info", "native data dir %s cache %s", dataDir, cacheDir)
}

//export Java_com_wails_app_WailsBridge_nativeInit
func Java_com_wails_app_WailsBridge_nativeInit(env *C.JNIEnv, obj C.jobject, bridge C.jobject, jdataDir C.jstring, jcacheDir C.jstring) {
	// Store JavaVM and bridge global reference for JNI callbacks
	C.storeBridgeRef(env, bridge)

	cData := C.jstringToC(env, jdataDir)
	dataDir := C.GoString(cData)
	C.releaseJString(env, jdataDir, cData)

	cCache := C.jstringToC(env, jcacheDir)
	cacheDir := C.GoString(cCache)
	C.releaseJString(env, jcacheDir, cCache)

	applyAndroidProcessEnv(dataDir, cacheDir)
	...
	if mainFunc != nil {
		androidMainOnce.Do(func() {
			go func() {
				defer func() {
					if r := recover(); r != nil {
						androidLogf("error", "Go main panic: %v", r)
					}
				}()
				mainFunc()
			}()
...
func javaBytesToGo(env *C.JNIEnv, arr C.jbyteArray) []byte {
	var n C.jint
	ptr := C.jbyteArrayElems(env, arr, &n)
	if ptr == nil || n <= 0 {
		return nil
	}
	defer C.releaseJByteArray(env, arr, ptr)
	return C.GoBytes(unsafe.Pointer(ptr), C.int(n))
}

//export Java_com_wails_app_WailsBridge_nativeServeAsset
func Java_com_wails_app_WailsBridge_nativeServeAsset(env *C.JNIEnv, obj C.jobject, jpath C.jstring, jmethod C.jstring, jheaders C.jstring, jbody C.jbyteArray) C.jbyteArray {
	cPath := C.jstringToC(env, jpath)
	cMethod := C.jstringToC(env, jmethod)
	cHeaders := C.jstringToC(env, jheaders)

	goPath := C.GoString(cPath)
	goMethod := C.GoString(cMethod)
	goHeaders := C.GoString(cHeaders)
	goBody := javaBytesToGo(env, jbody)

	C.releaseJString(env, jpath, cPath)
	C.releaseJString(env, jmethod, cMethod)
	C.releaseJString(env, jheaders, cHeaders)
	...
	if !waitForAppReady(10 * time.Second) {
		...
		return packedAndroidAssetJNI(env, 504, nil, nil)
	}
	...
	if app == nil || app.assets == nil {
		...
		return packedAndroidAssetJNI(env, 503, nil, nil)
	}

	data, status, hdr, err := serveAssetForAndroid(app, goMethod, goPath, goHeaders, goBody)
	if err != nil {
		androidLogf("error", "[JNI] error serving asset %s: %v", goPath, err)
		return packedAndroidAssetJNI(env, 500, nil, nil)
	}

	if status == 0 {
		status = http.StatusOK
	}
	return packedAndroidAssetJNI(env, status, hdr, data)
}

func packedAndroidAssetJNI(env *C.JNIEnv, status int, hdr http.Header, body []byte) C.jbyteArray {
	packed := packAndroidAssetResponse(status, hdr, body)
	if len(packed) == 0 {
		return C.createByteArray(env, nil, 0)
	}
	return C.createByteArray(env, unsafe.Pointer(&packed[0]), C.int(len(packed)))
}

func packAndroidAssetResponse(status int, hdr http.Header, body []byte) []byte {
	...
	out := make([]byte, 12+len(hj)+len(body))
	copy(out[0:4], []byte("JJAR"))
	binary.BigEndian.PutUint32(out[4:8], uint32(status))
	binary.BigEndian.PutUint32(out[8:12], uint32(len(hj)))
	copy(out[12:], hj)
	copy(out[12+len(hj):], body)
	return out
}
...
func serveAssetForAndroid(app *App, method string, path string, headersJSON string, reqBody []byte) ([]byte, int, http.Header, error) {
	...
	if app.assets == nil {
		return nil, 0, nil, fmt.Errorf("asset server not initialized")
	}
	...
	req, err := http.NewRequest(method, "https://wails.localhost"+path, bodyReader)
	if err != nil {
	...
	} else if strings.Contains(strings.ToLower(req.Header.Get("Content-Type")), "application/json") {
		androidLogf("warn", "[serveAsset] %s %s has Content-Type JSON but an empty body. Android WebView drops fetch() bodies unless they are stashed first", method, path)
	}
	...
	if err != nil {
		return nil, result.StatusCode, result.Header, fmt.Errorf("failed to read response body: %w", err)
	}
...
// shouldInterceptRequest. API bodies are stashed via JavascriptInterface
// (X-Wails-Body-Id) and attached here. Runtime calls go through
// nativeHandleRuntimeCall.
