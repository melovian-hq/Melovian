...
- [Task](https://taskfile.dev/) (optional; every `task` target wraps commands documented in the guides)
...
- JDK 21 for Gradle
- Xcode 15+ for iOS
- `bash build/scripts/ensure-go-mte.sh` builds a pinned Go tip with ARM MTE fixes. Desktop and server builds keep the Go version in `go.mod`.
